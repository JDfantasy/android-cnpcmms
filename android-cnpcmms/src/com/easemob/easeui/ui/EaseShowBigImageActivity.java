		...
		save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (uri != null && new File(uri.getPath()).exists()) {
					...
					File f = new File(Environment.getExternalStorageDirectory()+"/cnpc/image/", file.getName()+".jpg");
					if (!f.exists()) {
						...
						OutputStream fosto = new FileOutputStream(f);
						...
						fosfrom.close();
						fosto.close();
						Toast.makeText(EaseShowBigImageActivity.this, "已保存至："+f.getPath().replace(Environment.getExternalStorageDirectory()+"", "SD卡"), Toast.LENGTH_LONG).show();
						} catch (Exception ex) 
						{

...
public class Petrochina_search_stationentryActivity extends MyActivity {
	...
	private void set_view() {
		...
		message = (EditText) findViewById(R.id.editText_petrochina_search_stationtry_message);// 信息
		seek = (TextView) findViewById(R.id.textview_petrochina_search_stationtry_seek);// 搜索按钮
		seek.setOnClickListener(l);
		hint = (TextView) findViewById(R.id.textView_petrochina_search_stationtry_hint);// 提示信息
	...
	private View.OnClickListener l = new View.OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.imageView_myacitity_zuo:// 返回按钮
				...
				break;
			case R.id.textview_petrochina_search_stationtry_seek:// 搜索按钮
				hint.setVisibility(View.INVISIBLE);
				if (message.getText().toString().equals("")) {
					Toast.makeText(Petrochina_search_stationentryActivity.this, "请先输入关键词，在进行搜索", 0).show();
				} else {
					searchexamineentry();
				}
		...
		StringRequest re = new StringRequest(Request.Method.POST, Myconstant.PETROCHINA_GET_SEARCHPSTATIONENTRYSTATS,
				new Response.Listener<String>() {
					...
					public void onResponse(String response) {
						try {
							...
							if (js1.getString("type").equals("success")) {
								...
								if (body.length() == 0) {
									Toast.makeText(Petrochina_search_stationentryActivity.this, "对不起没有搜索到你要查询的供应商", 0)
											.show();
								} else {
					...
					public void onErrorResponse(VolleyError error) {
						...
						Toast.makeText(Petrochina_search_stationentryActivity.this, "你已经掉线，请重新登录", 0).show();
					}
				}) {
			@Override

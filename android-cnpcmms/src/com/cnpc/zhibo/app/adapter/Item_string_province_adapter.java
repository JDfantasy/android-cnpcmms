...
public class Item_string_province_adapter extends MyBaseAdapter<Centre_Addres_item> {
	private Context c;
	private int id = -1;

	public Item_string_province_adapter(Context context) {
		super(context);
		this.c = context;
	...
	public View setView(int position, View v, ViewGroup parent) {
		...
		if (id == position) {
			h.name.setTextColor(c.getResources().getColor(R.color.qianlvse));
